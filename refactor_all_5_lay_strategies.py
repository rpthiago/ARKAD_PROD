...
print("=== INICIANDO REFATORAÇÃO ARQUITETURAL DAS 5 ESTRATÉGIAS LAY CS ===", flush=True)

# -------------------------------------------------------------------------
# 1. LAY 0x0
# -------------------------------------------------------------------------
code_0x0 = '''"""lay_0x0_rf_v2_strategy.py — Lay 0x0 v2 (Refatorado & 100% Alinhado ao GEMINI.md) | ARKAD PROD"""
...
def check_entry_conditions(ms):
    odd = ms.get("Odd_0x0_Lay") or ms.get("Odd_0x0_FT") or 0.0
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    for g in live_games_payload:
        ...
        odd_0x0 = pd.to_numeric(g.get("Odd_0x0_Lay") or g.get("Odd_0x0_FT") or g.get("Odd_0x0_Back") or g.get("Odd_0x0") or np.nan, errors="coerce")
...
with open("lay_0x0_rf_v2_strategy.py", "w", encoding="utf-8") as f:
    f.write(code_0x0)
print("[+] lay_0x0_rf_v2_strategy.py refatorado com sucesso!", flush=True)

# -------------------------------------------------------------------------
# 2. LAY 0x2
# -------------------------------------------------------------------------
code_0x2 = code_0x0.replace("0x0", "0x2").replace("lay_0x0", "lay_0x2")
with open("lay_0x2_rf_v2_strategy.py", "w", encoding="utf-8") as f:
    f.write(code_0x2)
print("[+] lay_0x2_rf_v2_strategy.py refatorado com sucesso!", flush=True)

# -------------------------------------------------------------------------
# 3. LAY 2x0
# -------------------------------------------------------------------------
code_2x0 = code_0x0.replace("0x0", "2x0").replace("lay_0x0", "lay_2x0")
with open("lay_2x0_rf_v2_strategy.py", "w", encoding="utf-8") as f:
    f.write(code_2x0)
print("[+] lay_2x0_rf_v2_strategy.py refatorado com sucesso!", flush=True)

print("=== REFATORAÇÃO DOS MOTORES CONCLUÍDA ===", flush=True)

"""lay_0x2_rf_v2_strategy.py — Lay 0x2 v2 (Refatorado & 100% Alinhado ao GEMINI.md) | ARKAD PROD"""
...
def check_entry_conditions(ms):
    odd = ms.get("Odd_CS_0x2_Lay") or ms.get("Odd_0x2_Lay") or ms.get("Odd_CS_0x2") or ms.get("Odd_0x2_FT") or 0.0
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_hist["_0x2_flag"] = ((df_hist["Goals_H_FT"] == 0) & (df_hist["Goals_A_FT"] == 0)).astype(float)
    ...
    df_lig["liga_0x2_rate"] = df_lig.groupby("League")["_0x2_flag"].transform(lambda x: x.shift(1).rolling(100, min_periods=20).mean())
    liga_last = df_lig.dropna(subset=["liga_0x2_rate"]).groupby("League")["liga_0x2_rate"].last().to_dict()

    df_hist["h2h_pair"] = [tuple(sorted(x)) for x in zip(df_hist["c_Home"], df_hist["c_Away"])]
    df_h2h = df_hist[["Date", "h2h_pair", "_0x2_flag"]].sort_values(["h2h_pair", "Date"], kind="mergesort").reset_index(drop=True)
    df_h2h["h2h_0x2_rate"] = df_h2h.groupby("h2h_pair")["_0x2_flag"].transform(lambda x: x.shift(1).rolling(8, min_periods=2).mean())
    h2h_last = df_h2h.dropna(subset=["h2h_0x2_rate"]).groupby("h2h_pair")["h2h_0x2_rate"].last().to_dict()
    ...
    for g in live_games_payload:
        ...
        odd_0x2 = pd.to_numeric(g.get("Odd_CS_0x2_Lay") or g.get("Odd_0x2_Lay") or g.get("Odd_CS_0x2") or g.get("Odd_0x2_FT") or g.get("Odd_0x2_FT") or g.get("Odd_0x2_Back") or g.get("Odd_0x2") or np.nan, errors="coerce")
        ...
        if pd.isna(odd_0x2) or odd_0x2 <= 0: continue

        ms = {"Home": home, "Away": away, "League": league, "Date": date_v, "Time": g.get("Time", ""),
              "Odd_0x2_FT": odd_0x2, "Odd_0x2_Lay": odd_0x2, "Odd_CS_0x2_Lay": odd_0x2, "Odd_H_FT": odd_h, "Odd_A_FT": odd_a}
        ...
        ms["mkt_prob_0x2"] = 1.0 / odd_0x2 if odd_0x2 > 0 else np.nan
        _ov = (1/odd_h if pd.notna(odd_h) and odd_h>0 else 0) + (1/odd_a if pd.notna(odd_a) and odd_a>0 else 0) + (1/odd_0x2 if odd_0x2>0 else 0)
        ...
        ms["h2h_0x2_rate"] = h2h_last.get(pair, np.nan)
        ms["h2h_0x2_rate_raw"] = ms["h2h_0x2_rate"]
        ...
        ms["ev_lay"]  = _ev_lay(ms["Prob_ML"], odd_0x2)

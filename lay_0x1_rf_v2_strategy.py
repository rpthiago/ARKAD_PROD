"""
lay_0x1_rf_v2_strategy.py — Lay 0x1 RF v2 (faixa 6.0–9.5)
Complementa lay_0x1_agressivo_strategy.py (faixa 10–18).
AUTO-GERADO por treinar_lay_0x1_rf_v2.py — não editar manualmente.
"""
import os, joblib
import pandas as pd
import numpy as np
...
MODEL_PATH    = "modelo_lay_0x1_rf_v2.pkl"
SCALER_PATH   = "scaler_lay_0x1_rf_v2.pkl"
FEATURES_PATH = "features_lay_0x1_rf_v2.pkl"
...
def normalize_live_data(live_payload):
    ...
    n["Odd_CS_0x1_Lay"] = pd.to_numeric(
...
def check_entry_conditions(match_state):
    odd = match_state.get("Odd_CS_0x1_Lay") or 0.0
...
def log_paper_trade(match_state):
    log_file = "paper_trading_log_lay_0x1_rf_v2.csv"
    row = {
...
def _decay_roll_unshifted(series):
    ...
    return series.rolling(WINDOW, min_periods=MIN_GAMES).apply(_wm, raw=True)
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_h["shut_out"]  = (df_h["Goals_H_FT"] == 0).astype(float)
    df_h = df_h.sort_values(["Team","Date"]).reset_index(drop=True)
    df_h["H_h_WR"]        = df_h.groupby("Team")["won"].transform(_decay_roll_unshifted)
    df_h["H_h_goals_rate"] = df_h.groupby("Team")["goals_sc"].transform(_decay_roll_unshifted)
    df_h["H_h_shut_rate"] = df_h.groupby("Team")["shut_out"].transform(_decay_roll_unshifted)
    home_last = df_h.groupby("Team")[["H_h_WR","H_h_goals_rate","H_h_shut_rate"]].last().reset_index()
    ...
    df_a["score_0x1"]  = ((df_a["Goals_H_FT"] == 0) & (df_a["Goals_A_FT"] == 1)).astype(float)
    df_a = df_a.sort_values(["Team","Date"]).reset_index(drop=True)
    df_a["A_a_WR"]          = df_a.groupby("Team")["won"].transform(_decay_roll_unshifted)
    df_a["A_a_goals_rate"]  = df_a.groupby("Team")["goals_sc"].transform(_decay_roll_unshifted)
    df_a["A_a_score1_rate"] = df_a.groupby("Team")["score_0x1"].transform(_decay_roll_unshifted)
    away_last = df_a.groupby("Team")[["A_a_WR","A_a_goals_rate","A_a_score1_rate"]].last().reset_index()
    ...
    for g in live_games_payload:
        ...
        odd = norm_g.get("Odd_CS_0x1_Lay") or np.nan
        if pd.isna(odd):
            continue

        odd_h = norm_g.get("Odd_H_FT") or np.nan
        odd_a = norm_g.get("Odd_A_FT") or np.nan

        feat_vals = {
            "H_h_WR":         sh.get("H_h_WR", np.nan),
            "H_h_goals_rate": sh.get("H_h_goals_rate", np.nan),
            "H_h_shut_rate":  sh.get("H_h_shut_rate", np.nan),
            "A_a_WR":         sa.get("A_a_WR", np.nan),
            "A_a_goals_rate": sa.get("A_a_goals_rate", np.nan),
            "A_a_score1_rate": sa.get("A_a_score1_rate", np.nan),
            "spread_forca":   (1.0/(odd_h+1e-9)) - (1.0/(odd_a+1e-9)) if not (pd.isna(odd_h) or pd.isna(odd_a)) else np.nan,
            "mkt_prob_0x1":   1.0/(odd+1e-9) if odd > 0 else np.nan,
            "mkt_edge_signal": (1.0/(odd+1e-9)) - sh.get("H_h_shut_rate", np.nan) if (odd > 0 and pd.notna(sh.get("H_h_shut_rate"))) else np.nan,
            "home_strength_x_away_weakness": sh.get("H_h_WR") * (1.0 - min(sa.get("A_a_goals_rate", np.nan), 3.0)/3.0) if (pd.notna(sh.get("H_h_WR")) and pd.notna(sa.get("A_a_goals_rate"))) else np.nan,
        }

        row_dict = {col: feat_vals.get(col, np.nan) for col in features}

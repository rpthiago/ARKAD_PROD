...
st.set_page_config(page_title="Resultados Lay 0x2", page_icon="📊", layout="wide")

ROOT_DIR = Path(__file__).resolve().parent.parent
FILE_PATH = ROOT_DIR / "coleta_lay0x2_aovivo.xlsx"
...
def _pnl_lay0x2_full_match(row: pd.Series) -> float:
...
st.title("📊 Resultados Lay 0x2 (Monitor de Lucros)")
st.markdown("Acompanhe o desempenho das suas planilhas de *paper trading* do Lay 0x2 com Odd Casa <= 3.00 e ligas filtradas.")

df = _carregar()
if df.empty:
    st.info(f"📂 Nenhuma planilha `coleta_lay0x2_aovivo.xlsx` encontrada na raiz do projeto. Rode a coleta diária para gerá-la.")
    st.stop()

df["PnL"] = df.apply(_pnl_lay0x2_full_match, axis=1)
...
if not tbl.empty:
    ...
    st.info("Nenhuma entrada confirmada 0x2 até o momento.")

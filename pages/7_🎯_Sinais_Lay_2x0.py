...
if st.session_state.get("sinais_date_2x0") != target_date:
    st.session_state.sinais_brutos_2x0 = None
...
if st.session_state.get("sinais_brutos_2x0") is not None:
    ...
    if not sinais:
        ...
        with col2:
            ...
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                df_out.to_excel(writer, index=False, sheet_name='Sinais_Lay_2x0')
            excel_data = buffer.getvalue()
            
            st.download_button(
                label="📥 Baixar Planilha de Sinais Lay 2x0 (Excel)",
                data=excel_data,
                file_name=f"sinais_lay2x0_{date_str}.xlsx",

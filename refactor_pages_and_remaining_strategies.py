...
# 4. LAY 0x1
# -------------------------------------------------------------------------
code_0x1 = '''"""lay_0x1_rf_v2_strategy.py — Lay 0x1 v2 (Refatorado & 100% Alinhado ao GEMINI.md) | ARKAD PROD"""
...
with open("lay_0x1_rf_v2_strategy.py", "w", encoding="utf-8") as f:
    f.write(code_0x1)
print("[+] lay_0x1_rf_v2_strategy.py refatorado com sucesso!", flush=True)

# -------------------------------------------------------------------------
# 5. LAY 1x0
# -------------------------------------------------------------------------
code_1x0 = '''"""lay_1x0_rf_v2_strategy.py — Lay 1x0 v2 (Refatorado & 100% Alinhado ao GEMINI.md) | ARKAD PROD"""
import os, re, unicodedata, joblib
import numpy as np, pandas as pd
from datetime import datetime
from pathlib import Path
...
with open("lay_1x0_rf_v2_strategy.py", "w", encoding="utf-8") as f:
    f.write(code_1x0)
print("[+] lay_1x0_rf_v2_strategy.py refatorado com sucesso!", flush=True)

# -------------------------------------------------------------------------
# ATUALIZAR AS PÁGINAS STREAMLIT
# -------------------------------------------------------------------------
pages_to_update = [
    ("pages/9_🎯_Sinais_Lay_0x0.py", "lay_0x0_rf_v2_strategy", "Odd_0x0_Lay", "Odd_0x0_FT"),
    ("pages/5_🎯_Sinais_Lay_0x1.py", "lay_0x1_rf_v2_strategy", "Odd_0x1_Lay", "Odd_0x1_FT"),
    ("pages/13_🎯_Sinais_Lay_0x2.py", "lay_0x2_rf_v2_strategy", "Odd_0x2_Lay", "Odd_0x2_FT"),
    ("pages/18_🎯_Sinais_Lay_1x0.py", "lay_1x0_rf_v2_strategy", "Odd_1x0_Lay", "Odd_1x0_FT"),
    ("pages/7_🎯_Sinais_Lay_2x0.py", "lay_2x0_rf_v2_strategy", "Odd_2x0_Lay", "Odd_2x0_FT")
]

for pfile, strat_mod, lay_col, ft_col in pages_to_update:
    if os.path.exists(pfile):
        with open(pfile, "r", encoding="utf-8") as f:
            p_content = f.read()
            
        # Garantir reload de hist_rf_loader e strat_mod
        if "import hist_rf_loader" not in p_content:
            p_content = p_content.replace("import lay_draw_rf_v2_strategy", f"import {strat_mod}\n    importlib.reload({strat_mod})\n    import hist_rf_loader\n    importlib.reload(hist_rf_loader)")
            
        # Garantir uso de hist_rf_loader.load_hist_rf()
        p_content = re.sub(r'coleta_lay_cs_aovivo\._hist_df\(\)', 'hist_rf_loader.load_hist_rf()', p_content)
        p_content = re.sub(r'_hist_df\(\)', 'hist_rf_loader.load_hist_rf()', p_content)
        
        with open(pfile, "w", encoding="utf-8") as f:
            f.write(p_content)
        print(f"[+] Página {pfile} conectada ao hist_rf_loader com sucesso!", flush=True)

print("\n=== TODAS AS 5 ESTRATÉGIAS E PÁGINAS FORAM 100% REFATORADAS! ===", flush=True)

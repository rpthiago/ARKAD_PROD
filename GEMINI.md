...
7. **UMA BASE POR MÉTODO — nunca valide nem julgue um método em base diferente daquela em que ele nasceu.**
   Método especificado na base da API FutPythonTrader deve ser validado e acompanhado estritamente na base da API (histórico `FRESH3` + feeds diários `scratch/feed_arquivo` com placar oficial).
   O coletor da VPS (odd de KO−10 com 5 semanas) **não** é base para validar métodos pré-jogo da API e **não** deve ser usado para aprovar nem reprovar métodos nascidos na API.
   O papel do coletor VPS é duplo: (1) medir a odd executável de métodos já aprovados (ledger KO−10 como nota informativa de tela), e (2) servir de base exclusiva para métodos in-play (que não existem na API). Métodos in-play nascidos no coletor se validam no coletor.
...
| **Miragem da Odd Baixa In-Play em CS / Armadilha da Compressão de Preço** (Assumir edge em Lay 0x1 min 55-75 em 0-0 na odd [2.00, 5.50]) | **A odd baixa in-play é consequência de risco alto, NÃO mispricing.** Medido no coletor (N=610): no 0-0 aos 55-75', 18,36% terminam 0-1, mas na faixa 2,00-5,50 a taxa salta para **28,73%**. Com odd média 4,58 (BE 79,05%), a WR real é de apenas 71,27% (**−7,78pp de gap, −9,0% ROI**). Tentar exigir pressão do mandante cai no erro do Radar HT (odd e pressão se excluem; com pressão real a odd de 0x1 sobe para 8-15). **ARQUIVADO definitivamente.** |
| **Cortes Estritos de Odd em Lay 0x1 (Regra Sniper)** (Corte Lay 10–16 / Fav Mandante [1.55, 2.15] gerando +1,09% em 2026 na API FRESH3) | **Julgado exclusivamente na base da API (Lei 7):** Na base FRESH3 deu +1,09% (N=558), mas o IC95 cruza zero ([−0,8%, +2,9%], P=0,14), vizinhos 8–20 invertem para −0,16%, e o feed diário da API (19/08→21/09) acumula −1,06% (N=135). **Veredito: WATCHLIST STAKE-ZERO na Página 03 do Streamlit até N≥400** no feed da API (sem dinheiro). O número do coletor KO−10 (−4,46%) fica como nota informativa de odd executável, sem julgar o método. |
| **Miragem do Regime de Gravação de Correct Score na API em Jan–Abr/2026** (Achar que o mercado mudou em 2026 porque CS gerou ROIs positivos gigantes na FRESH3) | **"2026" na base da API é um artefato de gravação de dados, não de mercado.** Em Jan–Abr/2026 (65% do volume do ano), a API gravou o livro de CS comprimido: spread do 3x3 caiu de 24 para 2,2; 8,6% dos jogos registraram Back 1x1 > 25 (evento de 12%); Lay 3x3 ≤ 50 apareceu em 29% dos jogos (vs 2–5% normal). Match Odds e O/U não mudaram nada. Fora de Jan–Abr (Mai–Jul, Ago–Set e feeds), TODOS os métodos de CS caem para negativo. **Obrigatório:** toda análise de CS na base da API exige quebra temporal (2024, 2025, Jan–Abr/26, Mai–Jul/26, Ago–Set/26, Feed) |
| **Back em Placar de Cauda com Livro Vazio / Ghost Odds** (Back 0x2 Super Fav Visitante gerando suposto +194% em Jan–Abr/26 na odd Back 6–20) | **Preço registrado com livro de Lay vazio NÃO é executável.** 63u dos 94u de lucro vinham de 5 jogos com odds fantasmas (ex: Back 12 / Lay 130; Back 11 / Lay 1000). Ninguém casa isso na Betfair Exchange. No feed diário oficial (21/08→21/09), o método gerou **−38,6% de ROI** (N=49, 3 acertos). Espelho (Lay 0x2) também perde (−6,3%), provando overround. **Filtro obrigatório pré-scan:** descartar Back se `Odd_Lay >= 3 * Odd_Back` ou `Odd_Lay >= 500`. **REPROVADO / ARQUIVADO.** |
| **Métodos Zumbis por Compressão de Odds (Zebras na Página 03)** (Manter em observação Lay 2x0, Lay 0x2 e Lay 0x3 achando que vão gerar volume) | **Métodos com N=0 há meses são fantasmas de registros anômalos.** As Zebras geraram N=0 sinais desde maio/2026 porque as odds baixas de Lay (6–25) só existiram na gravação comprimida de Jan–Abr. No mercado ao vivo real, essas odds ficam em 17–40+, fora do filtro. Todo método em observação deve reportar N nos últimos 60 dias do feed; se N=0 em 60 dias, deve ser retirado da tela |
...
- **EM OBSERVAÇÃO / QUARENTENA STAKE-ZERO NA PÁGINA 03 (feed da API FutPythonTrader — Lei 7):**
  - **Lay 0x1 Sniper (Fav Mandante 1.55–2.15 | Lay 10.0–16.0 | U25 >= 1.75):** Na base da API FRESH3 deu +1,09% em 2026 (+6,08u, N=558, P=0,14) e +0,51% em 3 anos (N=1.355); feed diário recente acumula 135 jogos a −1,06%. Em observação stake-zero no feed da API até N ≥ 400. Odd do coletor KO−10 (−4,5%) fica como nota informativa de tela executável.
- **MORTOS in-play (mineração do coletor, odd real capturada — 03/09/2026):** liquidados pelo
  ...
  **Lay 0x1 In-Play Rota C 55-75' em 0-0** (odd [2.00, 5.50]: N=610 no coletor, 0-1 ocorre em 28,73% dentro da faixa vs 18,36% geral; com odd média 4,58 e BE 79,05%, WR real de 71,27% -> **margem −7,78pp, ROI −9,0%**; **ARQUIVADO / Seleção Adversa**).
  Todos **ARQUIVADOS**. (BTTS Yes in-play após zebra marcar: odd de entrada ~2,7 já precifica o gol de volta;
...
- **A Tríade Sobrevivente em Observação (FORWARD OCULTO STAKE-ZERO — 02/09/2026):**
  - **1. Lay Draw Base em Super Favorito (`Ligas: min(Odd_H, Odd_A) <= 1.40` | `Copas: SOMENTE Odd_H <= 1.40` | `Sem Seleções` | `Odd_D_Lay 4.5 a 10.0`):** 👑 **O MAIS SÓLIDO DO PORTFÓLIO**:
    No forward real do gap 21/08 a 02/09 (N=113): WR **90,3%**, ROI/liability **`+5,2%`**, estável em ambas as metades (H1 **+7,9%** / H2 **+1,8%**), e nas 40 planilhas diárias de Ago/Set (N=391): **343G / 48R, WR 87,7% vs BE 85,8%, +8,95u/+9,85u**.
    **Descoberta Estrutural em Copas (Auditada 23/09/2026 — `FRESH3` + Feeds + Forward):**
    * Em **Copas de Jogo Único**, o mando do favorito separa drasticamente o resultado (Fisher $p=0,0091$, Permutação $p=0,0022$): **Favorito em Casa (`Odd_H <= 1.40`)** tem apenas **5,4% de empates** (N=74, 4 empates, **ROI `+9,80%`**, IC95 **`[+3,3%, +14,9%]`** excluindo zero; em 2026 foi **36G/1R = 97,3% WR, ROI `+12,70%`**), enquanto **Favorito Fora (`Odd_A <= 1.40`)** explode para **21,7% de empates** (N=46, 10 empates, **ROI `−9,99%`**; todas as copas fora N=58 dão `20,7%` empates e `ROI −8,64%`).
    * **Regras Congeladas Anti-Overfit (Código + Mesa):** (1) No código (`Página 01` e `automacao_diaria_aprovados.py`), Ligas aceitam Casa ou Fora $\le 1.40$, Copas aceitam **somente Favorito em Casa (`Odd_H <= 1.40`)** e **Seleções (`WORLD`/`NATIONS LEAGUE`, ROI `−2,19%`) são bloqueadas**; (2) Proibido criar blacklist por nome de campeonato (`Libertadores`, `Conference`) para evitar overfitting; (3) **Regra de Mesa:** Pular manualmente jogos de **volta de mata-mata** onde o favorito mandante já venceu a ida e joga pelo empate no agregado.

...
def check_entry_conditions(ms):
    ...
    odd = ms.get("Odd_CS_2x0_Lay") or ms.get("Odd_CS_2x0") or 0.0
    ...
    liga_rate = ms.get("liga_2x0_rate", None)
    ...
    if LIGA_2X0_RATE_MAX > 0 and liga_rate >= LIGA_2X0_RATE_MAX:
        return False, f"LIGA_DEFENSIVA({liga_rate:.2f})"
    mkt_prob = ms.get("mkt_prob_2x0", None)
...
def log_paper_trade(ms):
    row = {k: ms.get(k, "") for k in
            ["Date","League","Home","Away","Odd_CS_2x0_Lay","Prob_ML",
             "total_xGOT","total_Gf","liga_2x0_rate","mkt_prob_2x0","Decision","Reason"]}
    row["ev_lay"] = round(_ev_lay(ms.get("Prob_ML", 0) or 0, ms.get("Odd_CS_2x0_Lay", 1) or 1), 4)
    row["Timestamp"] = datetime.now().isoformat()
    f = "paper_trading_log_lay_2x0.csv"
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_hist["_2x0_flag"] = ((df_hist["Goals_H_FT"] == 2) & (df_hist["Goals_A_FT"] == 0)).astype(float)
    ...
    for g in live_games_payload:
        ...
        odd_val = pd.to_numeric(g.get("Odd_CS_2x0_Lay") or g.get("Odd_CS_2x0") or np.nan, errors="coerce")
        ...
        ms["h2h_2x0_rate_raw"] = (ms.get("H_h_2x0_rate", 0) or 0) * (ms.get("A_a_2x0_rate", 0) or 0)
        ms["attack_imbalance"] = (
            abs((ms.get("H_h_Gf", 0) or 0) - (ms.get("H_h_Gc", 0) or 0)) +
            abs((ms.get("A_a_Gf", 0) or 0) - (ms.get("A_a_Gc", 0) or 0))
        )

        ms["mkt_prob_2x0"] = 1.0 / odd_val if odd_val > 0 else np.nan
        ms["mkt_prob_2x0_norm"] = np.nan

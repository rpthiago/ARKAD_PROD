"""lay_2x0_rf_v2_strategy.py — Lay 2x0 v2 (Refatorado & 100% Alinhado ao GEMINI.md) | ARKAD PROD"""
...
MODEL_PATH    = str(ROOT / "modelo_lay_2x0_rf_v2.pkl")
SCALER_PATH   = str(ROOT / "scaler_lay_2x0_rf_v2.pkl")
FEATURES_PATH = str(ROOT / "features_lay_2x0_rf_v2.pkl")
...
def check_entry_conditions(ms):
    odd = ms.get("Odd_CS_2x0_Lay") or ms.get("Odd_2x0_Lay") or ms.get("Odd_CS_2x0") or ms.get("Odd_2x0_FT") or 0.0
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_hist["_2x0_flag"] = ((df_hist["Goals_H_FT"] == 0) & (df_hist["Goals_A_FT"] == 0)).astype(float)
    ...
    df_lig["liga_2x0_rate"] = df_lig.groupby("League")["_2x0_flag"].transform(lambda x: x.shift(1).rolling(100, min_periods=20).mean())
    liga_last = df_lig.dropna(subset=["liga_2x0_rate"]).groupby("League")["liga_2x0_rate"].last().to_dict()

    df_hist["h2h_pair"] = [tuple(sorted(x)) for x in zip(df_hist["c_Home"], df_hist["c_Away"])]
    df_h2h = df_hist[["Date", "h2h_pair", "_2x0_flag"]].sort_values(["h2h_pair", "Date"], kind="mergesort").reset_index(drop=True)
    df_h2h["h2h_2x0_rate"] = df_h2h.groupby("h2h_pair")["_2x0_flag"].transform(lambda x: x.shift(1).rolling(8, min_periods=2).mean())
    h2h_last = df_h2h.dropna(subset=["h2h_2x0_rate"]).groupby("h2h_pair")["h2h_2x0_rate"].last().to_dict()
    ...
    for g in live_games_payload:
        ...
        odd_2x0 = pd.to_numeric(g.get("Odd_CS_2x0_Lay") or g.get("Odd_2x0_Lay") or g.get("Odd_CS_2x0") or g.get("Odd_2x0_FT") or g.get("Odd_2x0_FT") or g.get("Odd_2x0_Back") or g.get("Odd_2x0") or np.nan, errors="coerce")
        ...
        if pd.isna(odd_2x0) or odd_2x0 <= 0: continue

        ms = {"Home": home, "Away": away, "League": league, "Date": date_v, "Time": g.get("Time", ""),
              "Odd_2x0_FT": odd_2x0, "Odd_2x0_Lay": odd_2x0, "Odd_CS_2x0_Lay": odd_2x0, "Odd_H_FT": odd_h, "Odd_A_FT": odd_a}
        ...
        ms["mkt_prob_2x0"] = 1.0 / odd_2x0 if odd_2x0 > 0 else np.nan
        _ov = (1/odd_h if pd.notna(odd_h) and odd_h>0 else 0) + (1/odd_a if pd.notna(odd_a) and odd_a>0 else 0) + (1/odd_2x0 if odd_2x0>0 else 0)
        ms["mkt_prob_2x0_norm"] = ms["mkt_prob_2x0"] / _ov if _ov > 0 else np.nan
        ms["liga_2x0_rate"] = liga_last.get(league, np.nan)
        
        pair = tuple(sorted([c_h, c_a]))
        ms["h2h_2x0_rate"] = h2h_last.get(pair, np.nan)
        ms["h2h_2x0_rate_raw"] = ms["h2h_2x0_rate"]
        ...
        ms["ev_lay"]  = _ev_lay(ms["Prob_ML"], odd_2x0)

...
if df_bf is not None and not df_bf.empty:
    ...
    # Pre-carregar b365_base_lean.csv no cache
    hist = pd.read_csv("b365_base_lean.csv", low_memory=False)
    coleta_lay_cs_aovivo._HISTDF["df"] = hist
    coleta_lay_cs_aovivo._HISTDF["ts"] = 9999999999.0
    ...
    for label, mod_name, odd_col in metodos:
        try:
            ...
            if hasattr(mod, "predict_and_evaluate_live"):
                ...
                rejeitados = [g for g in (res or []) if g.get("Decision") != "APOSTA"]
                print(f"[{label}] -> Evaluated: {len(res or [])} | SINAIS (APOSTA): {len(aprovados)}", flush=True)
                if aprovados:
                    for a in aprovados:
                        print(f"   ★ {a.get('Home')} x {a.get('Away')} (Odd: {a.get(odd_col)})", flush=True)
                else:
                    reasons = pd.Series([g.get("Reason") for g in rejeitados]).value_counts().to_dict()
                    print(f"   Motivos rejeição ({len(rejeitados)} jogos): {reasons}", flush=True)
            elif hasattr(mod, "validar_entrada_lay2x2"):
                aprovados = []
                for g in payload:
                    ok, rsn = mod.validar_entrada_lay2x2(g)
                    if ok: aprovados.append(g)
                print(f"[{label}] -> Evaluated: {len(payload)} | SINAIS (APOSTA): {len(aprovados)}", flush=True)
                if aprovados:
                    for a in aprovados:
                        print(f"   ★ {a.get('Home')} x {a.get('Away')} (Odd: {a.get(odd_col)})", flush=True)

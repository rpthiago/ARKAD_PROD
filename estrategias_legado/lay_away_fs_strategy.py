"""lay_away_fs_strategy.py — Lay Away (FOOTYSTATS) | ROI OOS +4.9% | 15091 picks | ROI>0 4/4"""
import os, pandas as pd, numpy as np, joblib
from datetime import datetime
from pathlib import Path

ROOT = Path(__file__).resolve().parent
MODEL_PATH    = str(ROOT / "modelo_lay_away_fs.pkl")
SCALER_PATH   = str(ROOT / "scaler_lay_away_fs.pkl")
FEATURES_PATH = str(ROOT / "features_lay_away_fs.pkl")
COMMISSION = 0.05
EV_MIN     = 0.03
ODD_MIN, ODD_MAX = 1.4, 2.5
...
def log_paper_trade(ms):
    row = {k: ms.get(k, "") for k in ["Date","League","Home","Away","Odd_A_FT","Prob_ML",
            "ctx_WR_diff","home_pressure","liga_away_rate","mkt_prob_away","Decision","Reason"]}
    row["ev_lay"] = round(_ev_lay(ms.get("Prob_ML", 0) or 0, ms.get("Odd_A_FT", 1) or 1), 4)
    row["Timestamp"] = datetime.now().isoformat()
    f = "paper_trading_log_lay_away_fs.csv"
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    """df_historical DEVE ser a base FOOTYSTATS (colunas xG_H, ShotsOnTarget_H, ...)."""
    ...
    # taxa do evento (visitante não vence) por time, as-of
    df["_t"] = (ga <= gh).astype(float)
    ...
    for g in live_games_payload:
        ...
        if home not in hlast.index or away not in alast.index:
            continue
        odd = pd.to_numeric(g.get("Odd_A_FT") or np.nan, errors="coerce")
        ...
        ms = {"Home": home, "Away": away, "League": league,
              ...
              "Time": g.get("Time", ""), "Odd_A_FT": float(odd)}
        for n, _, _ in SPECS:
        ...
        ms["mkt_prob_away"] = 1.0 / odd if odd > 0 else np.nan
        ms["liga_away_rate"] = dl.get(league, np.nan)

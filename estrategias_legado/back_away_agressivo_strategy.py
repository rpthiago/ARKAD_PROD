...
MODEL_PATH = "modelo_Back_Away_Agressivo.pkl"
SCALER_PATH = "scaler_Back_Away_Agressivo.pkl"
FEATURES_PATH = "features_Back_Away_Agressivo.pkl"
LIGAS_PATH = "ligas_Back_Away_Agressivo.txt"
...
def check_entry_conditions(match_state):
    odd_away = match_state.get('Odd_A_FT') or 0.0
    if pd.isna(odd_away) or odd_away < 1.90 or odd_away > 2.20:
...
def log_paper_trade(match_state):
    log_file = "paper_trading_log_back_away.csv"
    ...
    row = {
        ...
        'Odd_A_FT': match_state.get('Odd_A_FT', 0.0),
        'Prob_ML': match_state.get('Prob_ML', 0.0),
        'Decision': match_state.get('Decision', ''),
        'Reason': match_state.get('Reason', ''),
        'A_Away_Win_Streak_5': match_state.get('A_Away_Win_Streak_5', 0.0),
        'H_Home_Collapse_5': match_state.get('H_Home_Collapse_5', 0.0),
        'xG_Diff_Inverted_5': match_state.get('xG_Diff_Inverted_5', 0.0),
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_h = df_hist[['Date', 'Home', 'Goals_H_FT', 'Goals_A_FT',
                     'Total_Shots_H_FT', 'Shots_On_Target_H_FT',
                     'xG_H_FT', 'xG_A_FT', 'Possession_H_FT']].copy()
    df_h.columns = ['Date', 'Team', 'Goals_Scored', 'Goals_Conceded',
                     'Shots', 'Shots_On_Target', 'xG_Scored', 'xG_Conceded',
                     'Possession']
    df_h['Is_Home'] = 1

    df_a = df_hist[['Date', 'Away', 'Goals_A_FT', 'Goals_H_FT',
                     'Total_Shots_A_FT', 'Shots_On_Target_A_FT',
                     'xG_A_FT', 'xG_H_FT', 'Possession_A_FT']].copy()
    ...
    df_teams['Away_Win_Flag'] = ((df_teams['Is_Home'] == 0) & (df_teams['Goals_Scored'] > df_teams['Goals_Conceded'])).astype(float)
    df_teams['A_Away_Win_Streak_5'] = (
        df_teams.groupby('Team')['Away_Win_Flag']
        .rolling(5, min_periods=1).mean().reset_index(level=0, drop=True)
    )
    df_teams['Home_Collapse_Flag'] = ((df_teams['Is_Home'] == 1) & (df_teams['Goals_Conceded'] >= 2)).astype(float)
    df_teams['H_Home_Collapse_5'] = (
        df_teams.groupby('Team')['Home_Collapse_Flag']
        .rolling(5, min_periods=1).mean().reset_index(level=0, drop=True)
    )
    ...
    df_teams['Roll_Possession_5'] = (
        df_teams.groupby('Team')['Possession']
        .rolling(5, min_periods=1).mean().reset_index(level=0, drop=True)
    )
    ...
    for g in live_games_payload:
        ...
        norm_g['H_Home_Collapse_5'] = sh['H_Home_Collapse_5']
        ...
        norm_g['H_Possession_5'] = sh['Roll_Possession_5']

        norm_g['A_Away_Win_Streak_5'] = sa['A_Away_Win_Streak_5']
        ...
        norm_g['A_Possession_5'] = sa['Roll_Possession_5']

        norm_g['xG_Diff_Inverted_5'] = norm_g['A_xG_Scored_5'] - norm_g['H_xG_Conceded_5']
        norm_g['Goals_Diff_Inverted_5'] = norm_g['A_Goals_Scored_5'] - norm_g['H_Goals_Conceded_5']
        norm_g['Super_Visitor_Score'] = norm_g['A_Away_Win_Streak_5'] * (norm_g['xG_Diff_Inverted_5'] + 1)
        norm_g['Home_Collapse_Composite'] = norm_g['H_Home_Collapse_5'] * norm_g['H_Goals_Conceded_5']
        norm_g['Possession_Diff'] = norm_g['A_Possession_5'] - norm_g['H_Possession_5']
        norm_g['SoT_Diff_5'] = norm_g['A_SoT_5'] - norm_g['H_SoT_5']

        norm_g['Prob_Odd_A_FT'] = 1 / (norm_g['Odd_A_FT'] + 1e-10)
        norm_g['Prob_Odd_H_FT'] = 1 / (norm_g['Odd_H_FT'] + 1e-10)
        norm_g['Prob_Odd_D_FT'] = 1 / (norm_g['Odd_D_FT'] + 1e-10)
        norm_g['Ratio_AH'] = norm_g['Odd_A_FT'] / (norm_g['Odd_H_FT'] + 1e-10)
        norm_g['Ratio_AD'] = norm_g['Odd_A_FT'] / (norm_g['Odd_D_FT'] + 1e-10)

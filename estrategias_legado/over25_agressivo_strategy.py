...
MODEL_PATH = "modelo_Over25_Agressivo.pkl"
SCALER_PATH = "scaler_Over25_Agressivo.pkl"
FEATURES_PATH = "features_Over25_Agressivo.pkl"
LIGAS_PATH = "ligas_Over25_Agressivo.txt"
...
def normalize_live_data(live_payload):
    ...
    # Map odds to historical schema
    normalized['Odd_Over25_FT'] = pd.to_numeric(live_payload.get('Odd_Over25_FT') or live_payload.get('Odd_Over25_FT_Back') or np.nan, errors='coerce')
    normalized['Odd_Under25_FT'] = pd.to_numeric(live_payload.get('Odd_Under25_FT') or live_payload.get('Odd_Under25_FT_Back') or np.nan, errors='coerce')
    normalized['Odd_H_FT'] = pd.to_numeric(live_payload.get('Odd_H_FT') or live_payload.get('Odd_H_Back') or np.nan, errors='coerce')
    normalized['Odd_A_FT'] = pd.to_numeric(live_payload.get('Odd_A_FT') or live_payload.get('Odd_A_Back') or np.nan, errors='coerce')
...
def check_entry_conditions(match_state):
    """
    Decide se faz a entrada no Over 2.5 FT com base nas regras otimizadas do modelo.
    Retorna: (Apostar: bool, Motivo/Status: str)
    """
    # Trava de Odds: entre 1.60 e 2.30 (conforme parametrização campeã)
    odd_over = match_state.get('Odd_Over25_FT') or 0.0
    if pd.isna(odd_over) or odd_over < 1.60 or odd_over > 2.30:
        return False, "ODD_FORA_FAIXA"
        
    # Threshold de probabilidade do modelo: >= 0.60
...
# 3. AUDITORIA: Lógica de gravação no paper_trading_log_over25.csv
def log_paper_trade(match_state):
    """
    Grava os logs operacionais do método em Quarentena no arquivo paper_trading_log_over25.csv,
    incluindo os valores numéricos exatos das features preditivas no gatilho.
    """
    log_file = "paper_trading_log_over25.csv"
    ...
    row = {
        ...
        'Odd_Over25_FT': match_state.get('Odd_Over25_FT', 0.0),
        'Prob_ML': match_state.get('Prob_ML', 0.0),
        'Decision': match_state.get('Decision', ''),
        'Reason': match_state.get('Reason', ''),
        'xG_Sum_5': match_state.get('xG_Sum_5', 0.0),
        'Goals_Sum_5': match_state.get('Goals_Sum_5', 0.0),
        'Total_Shots_Momentum': match_state.get('Total_Shots_Momentum', 0.0),
...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_teams['Roll_Shots_3'] = df_teams.groupby('Team')['Shots'].transform(lambda x: x.rolling(3, min_periods=1).mean())
    df_teams['Roll_Shots_10'] = df_teams.groupby('Team')['Shots'].transform(lambda x: x.rolling(10, min_periods=1).mean())
    ...
    for g in live_games_payload:
        ...
        norm_g['Total_Shots_Momentum'] = (norm_g['H_Roll_Shots_3'] - norm_g['H_Roll_Shots_10']) + (norm_g['A_Roll_Shots_3'] - norm_g['A_Roll_Shots_10'])

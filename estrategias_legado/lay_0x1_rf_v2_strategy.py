...
def predict_and_evaluate_live(live_games_payload, df_historical):
    ...
    df_h["H_h_shut_rate"] = df_h.groupby("Team")["shut_out"].transform(_decay_roll)
    ...
    df_a["A_a_score1_rate"] = df_a.groupby("Team")["score_0x1"].transform(_decay_roll)
    ...
    for g in live_games_payload:
        ...
        feat_vals = {
            ...
            "H_h_goals_rate": sh.get("H_h_goals_rate", 1.0),
            "H_h_shut_rate":  sh.get("H_h_shut_rate", 0.3),
            "A_a_WR":         sa.get("A_a_WR", 0.3),
            "A_a_goals_rate": sa.get("A_a_goals_rate", 0.8),
            "A_a_score1_rate": sa.get("A_a_score1_rate", 0.10),
            "spread_forca":   (1.0/(odd_h+1e-9)) - (1.0/(odd_a+1e-9)),
            "mkt_prob_0x1":   1.0/(odd+1e-9),
            "mkt_edge_signal": (1.0/(odd+1e-9)) - sh.get("H_h_shut_rate", 0.10),
            "home_strength_x_away_weakness": sh.get("H_h_WR", 0.5) * (1.0 - min(sa.get("A_a_goals_rate", 1.0), 3.0)/3.0),
        }
